...
export const WarehouseTransfer: React.FC<WarehouseTransferProps> = ({
    ...
    const [localSearch, setLocalSearch] = React.useState(transferForm.search || '');
    const [debouncedSearch, setDebouncedSearch] = React.useState(transferForm.search || '');
    ...
    }, [materials, debouncedSearch, receiptSearchClass, transferForm.fromWorkshop]);

    React.useEffect(() => {
        setVisibleCount(50);
    }, [debouncedSearch, receiptSearchClass, transferForm.fromWorkshop]);
            ...
            <div className="flex-1 grid grid-cols-12 gap-6 p-6 overflow-hidden">
                ...
                <div className="col-span-12 xl:col-span-4 flex flex-col gap-4 overflow-hidden">
                    <div className="bg-slate-50/50 dark:bg-slate-800/20 p-5 rounded-xl border border-slate-200/60 dark:border-slate-700/50 space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-3 relative">
                            <div className="space-y-1.5">
                                <label className="data-label text-slate-500 dark:text-slate-400 ml-1">Kho nguồn</label>
                                <select
                            ...
                            <div className="space-y-1.5">
                                <label className="data-label text-slate-500 dark:text-slate-400 ml-1">Kho đích</label>
                                <select
                                    className={`w-full px-3 py-2.5 bg-white dark:bg-[#0f172a] border rounded-xl font-bold text-sm text-slate-700 dark:text-slate-200 outline-none focus:border-sky-500 transition-all shadow-sm ${transferForm.fromWorkshop === transferForm.toWorkshop ? 'border-rose-300 dark:border-rose-900/50 focus:border-rose-500' : 'border-slate-200 dark:border-slate-700'}`}
                            ...
                            {transferForm.fromWorkshop === transferForm.toWorkshop && (
                                <p className="absolute -bottom-5 left-1 data-label text-rose-500 col-span-2">Kho nguồn và kho đích không được trùng nhau</p>
                            )}
                        </div>

                        <div className="grid grid-cols-2 gap-3">
                            <div className="space-y-1.5">
                                <label className="data-label text-sky-600 ml-1">Mã đơn hàng</label>
                            ...
                            <div className="space-y-1.5">
                                <label className="data-label text-sky-600 ml-1">Mã phiếu</label>
                    ...
                    <div className="flex-1 flex flex-col min-h-0 bg-white dark:bg-[#0f172a] rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden relative shadow-sm">
                        <div className="flex justify-between items-center p-4 border-b border-slate-100 dark:border-slate-700 bg-slate-50/30 dark:bg-slate-800/30">
                            <div className="flex items-center gap-2">
                                ...
                                <h4 className="section-title">Danh mục điều chuyển</h4>
                            </div>
                            <span className="px-2.5 py-1 bg-sky-100 dark:bg-sky-900/40 text-sky-700 dark:text-sky-300 rounded-full data-label">{(Array.isArray(transferForm.items) ? transferForm.items : []).length} Item</span>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto no-scrollbar space-y-2 p-3">
                            {Array.isArray(transferForm.items) && transferForm.items.length > 0 ? (
                                    ...
                                    return (
                                        <div key={idx} className="flex items-center justify-between p-3 bg-white dark:bg-[#1e293b] border border-slate-100 dark:border-slate-700 rounded-xl group hover:border-sky-200 dark:hover:border-sky-700 transition-all">
                                            <div className="min-w-0 flex-1 mr-3">
                                                ...
                                                <p className="data-label text-slate-400 dark:text-slate-500 mt-0.5">Tồn: <span className="text-slate-600 dark:text-slate-300 font-bold tabular-nums">{mat?.quantity}</span> {mat?.unit}</p>
                                            </div>
                                            <div className="flex items-center gap-2 shrink-0">
                                                <div className="flex items-center bg-slate-50 dark:bg-slate-800 rounded-lg p-1 border border-slate-200/50 dark:border-slate-700 h-9">
                                                    <button onClick={() => {
                                                    ...
                                                    }} className="p-1 text-slate-400 hover:text-red-500 dark:hover:text-red-400 transition-all w-7 h-7 flex items-center justify-center rounded-md hover:bg-white dark:hover:bg-slate-700 shadow-sm"><Minus size={12} /></button>
                                                    <input type="text" className="w-16 bg-transparent text-center text-sm font-bold text-slate-800 dark:text-white outline-none tabular-nums" value={item.quantity || ''} onChange={e => {
                                                    ...
                                                    <button onClick={() => {
                                                        ...
                                                        setTransferForm({ ...transferForm, items: newItems });
                                                    }} className="p-1 text-slate-400 hover:text-sky-600 dark:hover:text-sky-400 transition-all w-7 h-7 flex items-center justify-center rounded-md hover:bg-white dark:hover:bg-slate-700 shadow-sm"><Plus size={12} /></button>
                                                </div>
                                                <button
                                                    onClick={() => {
                                ...
                                <div className="h-full flex flex-col items-center justify-center text-slate-400 gap-3 opacity-80 pt-10">
                                    ...
                                    <p className="data-label text-slate-500 dark:text-slate-400 text-center px-8 leading-relaxed">Hãy chọn vật tư từ danh sách bên phải để thêm vào phiếu</p>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <button
                        ...
                        className="w-full py-4 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white rounded-2xl shadow-xl shadow-emerald-500/20 hover:shadow-emerald-500/40 active:scale-[0.98] transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:from-slate-300 disabled:to-slate-400 dark:disabled:from-slate-700 dark:disabled:to-slate-800 disabled:shadow-none"
                    >
                        <Check size={18} /> {canManage ? <span className="data-label text-white">Xác nhận điều chuyển</span> : <span className="data-label text-white">Không có quyền điều chuyển</span>}
                    </button>
                </div>
                ...
                <div className="col-span-12 xl:col-span-8 bg-slate-50 dark:bg-slate-800/30 rounded-[20px] p-6 flex flex-col overflow-hidden border border-slate-200/60 dark:border-slate-700/50 shadow-inner">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 shrink-0">
                        <div className="relative">
                            ...
                            <input type="text" placeholder="Tìm vật tư kho nguồn..." className="w-full pl-11 pr-4 py-3 bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-700 rounded-xl font-medium text-sm text-slate-800 dark:text-slate-200 outline-none shadow-sm focus:border-sky-500 transition-all uppercase" value={localSearch} onChange={e => setLocalSearch(e.target.value)} />
                        </div>
                        <div className="flex items-center gap-1 p-1 bg-white dark:bg-[#0f172a] rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-x-auto no-scrollbar">
                            <button onClick={() => setReceiptSearchClass('ALL')} className={`flex-1 py-2 rounded-lg data-label transition-all ${receiptSearchClass === 'ALL' ? 'bg-slate-800 text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`}>Tất cả</button>
                            <button onClick={() => setReceiptSearchClass('Vật tư chính')} className={`flex-1 py-2 rounded-lg data-label transition-all ${receiptSearchClass === 'Vật tư chính' ? 'bg-sky-600 text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`}>Chính</button>
                            <button onClick={() => setReceiptSearchClass('Vật tư phụ')} className={`flex-1 py-2 rounded-lg data-label transition-all ${receiptSearchClass === 'Vật tư phụ' ? 'bg-rose-500 text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`}>Phụ</button>
                        </div>
                    </div>
                                ...
                                return (
                                    <button key={m.id} onClick={() => {
                                        ...
                                        <div className="min-w-0 w-full">
                                            <h5 className="font-bold text-sm mb-2 group-hover:text-sky-700 dark:group-hover:text-sky-400 leading-tight uppercase line-clamp-2">{m.name}</h5>
                                            <span className={`px-1.5 py-0.5 ${m.classification === 'Vật tư chính' ? 'bg-sky-100/50 dark:bg-sky-900/40 text-sky-600 dark:text-sky-400' : 'bg-rose-100/50 dark:bg-rose-900/40 text-rose-500 dark:text-rose-400'} data-label rounded inline-block`}>{m.classification === 'Vật tư chính' ? 'Chính' : 'Phụ'}</span>
                                        </div>
                                        <div className="flex items-end justify-between mt-auto pt-2 border-t border-slate-50 dark:border-slate-700 w-full">
                                            <div>
                                                <p className="data-label text-slate-400 mb-0.5">Tồn kho</p>
                                                <p className="text-lg font-bold text-foreground tabular-nums">{formatNumber(m.quantity)} <span className="data-label text-slate-400">{m.unit}</span></p>
                                            </div>
